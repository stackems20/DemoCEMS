...
def run_model(df, target, training_file, model_file):
    ...
    for lag in [6, 12, 24]:
        df_new[f"{target}_lag_{lag}"] = df_new[target].shift(lag)
    ...
    df_new["weight"] = 1 + (df_new[target] / df_new[target].mean())
    ...
    if st.session_state.get("run", False):
        ...
        if "data" not in st.session_state:
            ...
            with st.spinner("Running models..."):
                ...
                nox = run_model(df, "NOx", "Nx_training_dataset.pkl", "tft_model_NOx.ckpt")
                so2 = run_model(df, "SO2", "training_dataset_SO2.pkl", "tft_model_SO2.ckpt")
                pm  = run_model(df, "PM", "training_dataset_PM.pkl", "tft_model_PM.ckpt")
